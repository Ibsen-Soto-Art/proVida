...
- [x] **Un experimento que "no funciona" a la primera es información, no un fracaso** — el primer diseño de la extensión de temperatura (4 genotipos con temperaturas muy separadas) se extinguía casi de inmediato; en vez de forzar los números hasta que "se viera bien", diagnostiqué la causa (desventaja inicial aplastante) y rediseñé el experimento con variación más realista.
- [x] **Un servidor persistente necesita salvaguardas que un script no necesita** — antes de desplegar `webapp/` (que mantiene una simulación viva por cada visitante conectado), agregué un límite de conexiones simultáneas y, al probarlo, encontré un `RuntimeError` sin capturar que ocurría cada vez que un cliente cerraba la conexión en el momento equivocado — se habría disparado con cada visitante real cerrando la pestaña, y solo apareció al simular cierres en ráfaga, no en el uso normal de un solo cliente.
- [x] **Desplegar por SSH en vivo, editando a mano, es propenso a errores incluso siguiendo instrucciones** — al añadir el bloque de Nginx para el WebSocket, terminó duplicado y en el bloque equivocado (el de redirección HTTP, no el HTTPS real). Se detectó pidiendo confirmar el archivo completo antes de recargar Nginx, y se corrigió reescribiéndolo entero de una sola vez en vez de seguir parchando a mano.
